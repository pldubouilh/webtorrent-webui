...
module.exports = function (q, wt) {
  ...
  switch (q.method) {
    case 'session-get':
      parsed = wt.getSession() // Return dummy session data
      break
    ...
    case 'torrent-remove':
      const rm = q.arguments['delete-local-data']
      q.arguments.ids.forEach(infoHash => wt.remove(infoHash, rm), wt)
      break
